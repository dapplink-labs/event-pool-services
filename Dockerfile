...
# 复制 GO 配置
# COPY GO.conf /opt/chooseme-services/
COPY event-pod-services-config.local.yaml /opt/chooseme-services/event-pod-services-config.local.yaml
...
CMD ["./event-pod-services","api","--config","./event-pod-services-config.local.yaml"]